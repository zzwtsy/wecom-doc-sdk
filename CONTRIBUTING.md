...
- 测试目录统一放在 `tests/`
- 测试分层：优先补充单元测试（模型校验、参数构造、错误映射）；需要真实链路验证时再补集成测试
- 新增接口应配套至少 1 个核心路径测试，并至少覆盖 1 个失败路径（如 `errcode != 0`、HTTP 非 2xx、超时、响应解析失败）
- 涉及模型约束时，至少补 1 个失败路径测试（例如二选一校验失败、条件必填缺失）
- 新增 API 同时支持 `dict` 与模型对象入参时，建议覆盖两种入参形式
- 单元测试禁止真实网络请求，统一通过 `respx` 或 mock `httpx` 隔离外部依赖；集成测试需使用 `@pytest.mark.integration` 标记
- 对多输入/多分支场景，优先使用 `@pytest.mark.parametrize`，避免重复样板测试
- 共享准备逻辑（客户端初始化、通用请求桩、测试数据）优先沉淀为 fixture，并显式声明合理作用域（`function`/`module`）
- 断言需聚焦业务语义：除“成功/失败”外，应校验关键字段、异常类型、错误码及请求参数构造结果
- 如涉及异步接口，统一采用 `pytest` 异步测试标记（如 `pytest.mark.asyncio`）并与同步测试分组管理
- 建议配置覆盖率基线（例如总覆盖率 `>=80%`），新增模块需重点覆盖核心路径与异常路径
